...
var AuthenticateProcessor = /** @class */ (function () {
    function AuthenticateProcessor(sessionToken, sampleAppControllerReference) {
        ...
        this.onFaceTecSDKCompletelyDone = function () {
            ...
            if (_this.success) {
                ...
                ///window.backend.onAuthSuccess(window.userPwHash);
                window.backend.authSuccess(window.backend.userHash);
            }
            else
            {
                window.backend.authFail(window.backend.userHash);
                ///window.backend.onAuthFail(window.userPwHash);
            }
    ...
    AuthenticateProcessor.prototype.processSessionResultWhileFaceTecSDKWaits = function (sessionResult, faceScanResultCallback) {
        ...
        if (sessionResult.status !== FaceTecSDK.FaceTecSessionStatus.SessionCompletedSuccessfully) {
            ...
            ///window.backend.onAuthCancel(window.userPwHash);
            window.backend.authCancel(window.backend.userHash);
    ...
    //alert("user hash passed is: " + userHash);
    //alert("user hash passed is: " + window.backend.jsGetUserHash);
	...
	var parameters = {
            ...
            //externalDatabaseRefID: "myUniqueID"
            //externalDatabaseRefID: "none-existingID"
            externalDatabaseRefID: window.backend.userHash ///window.userPwHash
        };
        //
